...
public class HttpRequestHandlerTest {
    ...
    @Test
    public void buildResponseHeaders_ignores_status_line() {
        Map<String, List<String>> headers = new HashMap<>();
        headers.put(null, Arrays.asList("HTTP/1.1 200 OK"));
        headers.put("Content-Type", Arrays.asList("text/plain"));

        JSObject result = HttpRequestHandler.buildResponseHeaders(headers);

        assertEquals("{\"Content-Type\":\"text/plain\"}", result.toString());
    }

...
const pluginInstanceRegistry: {
  [pluginClassName: string]: { [functionName: string]: any };
} = {};
    ...
    for (const classKey of Object.keys(plugins[pluginKey]).filter(
      ...
      if (pluginInstanceRegistry[classKey] instanceof EventEmitter) {
        ...
        ipcMain.on(`event-add-${classKey}`, (event, type) => {
          ...
          (pluginInstanceRegistry[classKey] as EventEmitter).addListener(
            type,
            eventHandler,
          );

          ipcMain.once(`event-remove-${classKey}-${type}`, () => {
            (pluginInstanceRegistry[classKey] as EventEmitter).removeListener(
              type,
              eventHandler,
            );
          });
...
export function getCapacitorElectronConfig(): CapacitorElectronConfig {
  ...
  if (existsSync(join(app.getAppPath(), 'build', 'capacitor.config.js'))) {
    capFileConfig = require(
      join(app.getAppPath(), 'build', 'capacitor.config.js'),
    ).default;
  } else {

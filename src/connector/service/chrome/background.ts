import {
    AddressResponsePayload, BalanceResponsePayload,
    ConfirmPayload,
    EventData,
    MessageContent,
    MessageData,
...
const sendMessage = (server: string, id: string, pageId: string, content: string) => {
    const msg = {
        action: 'send',
        payload: {
            client: id,
    ...
    if (msg.type === 'call') {
        ...
        if (session && session.port) {
            ...
            if (!session.walletId) {
                ...
                switch (fn) {
                    case 'address':
                        sendMessage(session.server, session.walletId, session.id, JSON.stringify({

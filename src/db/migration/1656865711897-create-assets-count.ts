...
export class createAssetsCount1656865711897 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        const oldBoxes = await queryRunner.query('SELECT * FROM box') as Array<Box>
        for(let item of oldBoxes){
            const boxJson = JSON.parse(item.json)
            const assetCount = boxJson.hasOwnProperty('assets') ? boxJson.assets.length : 0
            await queryRunner.query(`UPDATE box set asset_count=${assetCount} WHERE id=${item.id}`)
        }
    }

...
@ViewEntity({
  ...
  expression: (connection: DataSource) =>
    connection
      ...
      .addSelect(
        '(SELECT CAST(SUM(CAST(erg AS INT)) AS TEXT) from box WHERE box.txId = Tx.id)',
        'create_erg_str'
      )
      .addSelect(
        '(SELECT CAST(SUM(CAST(erg AS INT)) AS TEXT) from box WHERE box.spendTxId = Tx.id)',
        'spent_erg_str'
      )
      .from('tx', 'Tx')
      ...
      .leftJoin(
        'address',
        'CreateAddress',
        'CreateAddress.id=CreateBox.addressId'
      )

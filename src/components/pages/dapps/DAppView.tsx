...
class DAppView extends React.Component<DAppViewPropsType, DAppViewStateType> {
  ...
  getAddresses = (): Promise<Array<string>> => {
    return AddressDbAction.getWalletAddresses(this.props.wallet.id).then(
      (result) => result.map((item) => item.address)
    );
  };

  getCoveringForErgAndToken = async (
    amount: bigint,
    ...
    const addressObject = address
      ? (
          await AddressDbAction.getAddressByAddressString(address)
        )?.addressObject()
      : null;
    return BoxDbAction.getCoveringBoxFor(
      amount,
  ...
  getTokenAmount = async (tokenId?: string) => {
    if (tokenId) {
      const result =
        await BoxContentDbAction.getSingleTokenWithAddressForWallet(
          this.props.wallet.id,
          tokenId
        );
      return result
  ...
  descriptionIcon = () => {
    ...
    if (dApp && dApp.readme) {
      return (
        <IconButton
          color="inherit"
          onClick={() => this.setState({ show_description: true })}
        >
        ...
        <WithAppBar
          header={
            <AppHeader
              title={dApp.name}
              hideQrCode={true}
              extraIcons={this.descriptionIcon()}
            />
          }
        >
          {dApp.id === 'issueToken' ? (
          ...
          {dApp.id === 'sigmaUsd' ? <SigmaUSD {...this.getProps()} /> : null}
          ...
          {dApp && dApp.readme ? (
            <BottomSheet
              show={this.state.show_description}
              close={() => this.setState({ show_description: false })}
            >
              <Container>
                {dApp.readme}
                <br />

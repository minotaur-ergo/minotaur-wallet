...
const get_amount = (amount: string, token_type: 'USD' | 'RSV') => {
  if (token_type === 'USD') {
    ...
    const decimal_part =
      amount_parts.length <= 1 ? '00' : amount_parts[1].padStart(2, '0');
...
const BuySellToken = (props: PropsType) => {
  ...
  if (get_decimal(amount).length > decimal_length) {
    ...
    error = `Unable to mint more than ${format_usd(maxAmount)} Sigma${
      props.token_type
    } based on the current reserve status`;
  ...
  return (
    <React.Fragment>
      ...
      <FormHelperText>
        Maximum Available:{' '}
        {props.token_type === 'USD'
          ? format_usd(maxRedeem)
          : maxRedeem.toString()}
      </FormHelperText>
      {blurred ? (
      ...
      >
        {props.operation === 'BUY' ? 'Purchase' : 'Redeem'}
      </Button>
    </React.Fragment>
  );
};

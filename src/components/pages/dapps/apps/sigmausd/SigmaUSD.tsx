...
class SigmaUSD extends React.Component<DAppPropsType, SigmaUSDStateType> {
    ...
    getCoveringBox = async (amount: bigint, bank: Bank, token?: string, token_amount?: bigint) => {
        ...
        if (!boxes.covered) {
            this.props.showNotification("Not enough erg or tokens to process", "error")
            return undefined;
        }
    ...
    sell_token_tx = async (token_type: "stable" | "reserve", amount: bigint) => {
        if (!this.state.bank || !this.state.oracle) {
            this.props.showNotification("please wait to sync with blockchain", "error")
        }
    ...
    buy_stable = (amount: bigint) => {
        this.buy_token_tx("stable", amount).then(() => null).catch(err => this.props.showNotification(err, "error"));
    };

    sell_stable = (amount: bigint) => {
        this.sell_token_tx("stable", amount).then(() => null).catch(err => this.props.showNotification(err, "error"));
    };

    buy_reserve = (amount: bigint) => {
        this.buy_token_tx("reserve", amount).then(() => null).catch(err => this.props.showNotification(err, "error"));
    };

    sell_reserve = (amount: bigint) => {
        this.sell_token_tx("reserve", amount).then(() => null).catch(err => this.props.showNotification(err, "error"));
    };

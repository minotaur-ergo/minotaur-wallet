...
const WalletPage = (props: PropsType) => {
    ...
    useEffect(() => {
        if (!wallet || wallet.id + "" !== params.id) {
            const wallets = props.wallets.filter(item => "" + item.id === "" + params.id);
            if (wallets.length === 0) {
            ...
            <WalletQrCodeContext.Provider value={{
                ...
                <WithAppBar
                    ...
                    <div>
                        {wallet ? (
                            <Routes>
                                <Route path={WalletPageSuffix.WalletTransaction}
                                ...
                                <Route path={WalletPageSuffix.WalletDApps}
                                       element={<DAppList wallet={wallet} setTab={setCurrentTab}/>}/>
                                <Route path={WalletPageSuffix.WalletDAppView}
                        ...
                        <Paper sx={{position: 'fixed', bottom: 0, left: 0, right: 0}} elevation={3}>
                            ...
                            >
                                <BottomNavigationAction
                                    onClick={gotoPage(navigate, getRoute(RouteMap.WalletTransaction, {id: params.id}))}
                                    label="Transactions"
                                    icon={<FormatListBulletedOutlined/>}/>
                                <BottomNavigationAction
                                    onClick={gotoPage(navigate, getRoute(RouteMap.WalletSend, {id: params.id}))}
                                    label="Send"
                                    icon={<ReceiptOutlined/>}/>
                                <BottomNavigationAction
                                    onClick={gotoPage(navigate, getRoute(RouteMap.WalletAddress, {id: params.id}))}
                                    label="Addresses"
                                    icon={<ContactMailOutlined/>}/>
                                <BottomNavigationAction
                                    onClick={gotoPage(navigate, getRoute(RouteMap.WalletAssets, { id: params.id }))}
                                    label="Assets"
                                    icon={<AccountBalanceWallet />} />
                                <BottomNavigationAction
                                    onClick={gotoPage(navigate, getRoute(RouteMap.WalletDApps, {id: params.id}))}

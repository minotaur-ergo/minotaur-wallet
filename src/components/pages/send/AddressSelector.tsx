...
interface PropsType {
    wallet: Wallet;
    setParams: (amount: bigint, address: Array<Address> | null, tokens: Array<TokenWithAddress>) => any;
}
...
class AddressSelector extends React.Component<PropsType, StateType> {
    ...
    setSelectedAddress = (value: number | Array<number>) => {
        if(this.state.multiple) {
            ...
            const newSelected = value === -1 ? Array(this.state.addresses.length).fill(0).map((item, index) => index) : [value as number];
            this.setState({ selectedAddress: value as number, multipleSelectedAddresses: newSelected });
        }
    };
    ...
    getTotalErg = () => {
        // if(this.state.multiple){
        return this.state.addresses.filter(
            (item, index) => this.state.multipleSelectedAddresses.indexOf(index) >= 0
        ).map(item => item.erg()).reduce((a, b) => a + b, BigInt(0));
        // }
    ...
    loadExtraContent = async () => {
        if (this.props.wallet.id !== this.state.tokensForWallet) {
            ...
            const addresses = await AddressDbAction.getWalletAddressesWithErg(wallet_id);
            const tokens = await BoxContentDbAction.getTokenWithAddressForWallet(wallet_id);
            const multipleSelectedAddresses = this.state.multiple? this.state.multipleSelectedAddresses : this.state.selectedAddress === -1 ? Array(addresses.length).fill(0).map((item, index) => index) : [this.state.selectedAddress]
            this.setState({
                addresses: addresses,
                totalTokens: tokens,
    ...
    getSelectedAddresses = () => {
        if (this.state.multiple) {
            return this.state.addresses.filter((item, index) => this.state.multipleSelectedAddresses.indexOf(index) >= 0).map(item => item.addressObject());
        }
        return [(this.state.addresses[this.state.selectedAddress] as AddressWithErg).addressObject()];
    };

    checkNeedPublishState = () => {
        if(this.state.multiple !== this.state.published.multiple) return true
        if(this.state.multiple){
            if(this.state.multipleSelectedAddresses.length !== this.state.published.selectedArray.length) return true
            ...
            if (this.checkNeedPublishState()) {
                ...
                if (!multiple && selected === -1) {
                    ...
                    this.props.setParams(
                        ...
                        this.state.totalTokens.filter((item: TokenWithAddress) => addressIds.indexOf(item.address_id) > -1)
                    );
                }
                this.setState({
                    published: {
        ...
        return (
            <React.Fragment>
                <FormControl fullWidth variant="outlined" size="small" style={{ marginBottom: -10 }}>
                    ...
                    <Select
                        value={this.state.multiple ? this.state.multipleSelectedAddresses : this.state.selectedAddress}
                        label="From Address"
                    ...
                    <FormHelperText>
                        Balance:
                        <Erg erg={this.getTotalErg()} showUnit={true}
                             network_type={this.state.addresses.length ? this.state.addresses[0].network_type : ""} />
                        <FormControlLabel
                            style={{ float: "right" }}
                            control={
                                <Switch
                                    checked={this.state.multiple}
                                    onChange={event => this.setState({ multiple: event.target.checked })}
                                    name="multiple"
                                    color="primary"
                                />
                            }

...
class SendTransaction extends React.Component<
  SendTransactionPropsType,
  SendTransactionStateType
> {
  ...
  addReceiver = () => {
    this.setState((state) => ({
      ...state,
      receivers: [...state.receivers, new Receiver('', '')],
    }));
  };
  ...
  updateTotalParams = async () => {
    if (this.props.display === 'simple') {
      const totalErg = (
        await AddressDbAction.getWalletAddressesWithErg(this.props.wallet.id)
      )
        .map((item) => item.erg())
  ...
  checkAddressesEqual = (
    addrList1: Array<Address> | undefined | null,
    addrList2: Array<Address> | undefined | null
  ) => {
    if (addrList1 && addrList2) {
      if (addrList1.length !== addrList2.length) return false;
      return (
        addrList1.filter(
          (item, index) => getAddressId(item) !== getAddressId(addrList2[index])
        ).length === 0
      );
    }
  ...
  generateAndSendTx = async () => {
    try {
      const tx = await BlockChainAction.createTx(
        this.state.receivers,
        this.props.wallet,
        this.state.selectedAddress ? this.state.selectedAddress : undefined
      );
      this.setState({
        showModal: true,
        generatedTx: tx,
  ...
  renderSendButton = () => {
    const isValid =
      this.state.receivers.filter((item: Receiver) => !item.valid()).length ===
      0;
    const validAddress =

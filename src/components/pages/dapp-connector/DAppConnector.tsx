...
import {
    ActionType,
    AddressRequestPayload, BalanceRequestPayload,
    Connection,
    ConnectionData,
    ConnectionState,
    MessageContent,
...
class DAppConnector extends React.Component<DAppConnectorPropType, DAppConnectorStateType> {
    ...
    processAddress = async (connection: ConnectionState, content: MessageContent) => {
        ...
        if (wallet) {
            ...
            this.sendMessageToServer(connection, "address_response", content.requestId, resultAddress);
        }
    }
    ...
    processBalance = async (connection: ConnectionState, content: MessageContent) => {
        ...
        if(wallet) {
            ...
            this.sendMessageToServer(connection, "balance_response", content.requestId, res);
        }
    }
    handleMessage = (msg: MessageData) => {
        const filteredConnections = this.state.connections.filter(item => item.info.pageId === msg.pageId)
    ...
    sendMessageToServer = (connection: ConnectionState, action: ActionType, requestId: string, payload: Payload) => {
        const serverAddress = connection.info.server;
        let server: Connection = this.state.servers[serverAddress];
        server.send(connection.info.id, connection.info.pageId, JSON.stringify({
            action: action,
            requestId: requestId,
            payload: payload
        }))

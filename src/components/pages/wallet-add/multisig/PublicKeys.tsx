...
interface PublicKeysPropsType {
  ...
  goBack?: () => unknown;
  goForward: (wallet: number, public_keys: Array<string>) => unknown;
...
const TotalSign = (props: PublicKeysPropsType) => {
  ...
  const validPublicKey = (publicKey: string) => {
    if (
      is_valid_address(publicKey) ||
      get_base58_extended_public_key(publicKey)
    ) {
      const firstPublicKey = publicKeys[0];
      if (
        is_valid_address(firstPublicKey) ||
        get_base58_extended_public_key(firstPublicKey)
      ) {
  ...
  const formValid = () => {
    ...
    return publicKeys.length
      ? publicKeys.filter((item) => validPublicKey(item) === '').length > 0
      : false;
  };

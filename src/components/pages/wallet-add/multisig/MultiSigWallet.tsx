...
class MultiSigWallet extends WalletCreate {
    ...
    saveWallet = () => {
        if (!this.state.saving) {
            ...
            WalletAction.createMultiSigWallet(
                ...
                this.state.minSig
            ).then(res => this.goBack()).catch(err => {
    ...
    renderConfirm = () => (
        <PublicKeys
            ...
            goForward={(wallet: number, public_keys: Array<string>) => this.goMultiSigConfirm(wallet, public_keys)}>
        </PublicKeys>
    );
    ...
    renderMnemonic = () => {
        return (
            <TotalSign
                minSign={3}
                total={this.state.public_keys.length ? this.state.public_keys.length : 2}
                goBack={this.goBackName}
                goForward={(total: number, minSig: number) => this.goPublicKeys(total, minSig)}>
            </TotalSign>
        );
    };

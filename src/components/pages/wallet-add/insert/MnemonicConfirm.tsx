...
const MnemonicConfirm = (props: PropsType) => {
  ...
  const handleCreatedMnemonicClick = (index: number) => {
    setCreatedMnemonic(
      createdMnemonic
        .split(' ')
        .filter((item, wordIndex) => index !== wordIndex)
        .join(' ')
    );
  };
  const mnemonic = mnemonicWords.join(' ');
  const remainingWords = mnemonicWords.filter(
    (word: string, index: number) => createdIndex.indexOf(index) === -1
  );
  const createdHideIndex = Array(remainingWords.length)
    .fill(0)
    .map(
      (item: number, index: number) =>
        index + (createdMnemonic === '' ? 0 : createdMnemonic.split(' ').length)
    );
  return (
    <Container>
      <Grid container spacing={2}>
        <Grid item xs={12}>
          <MnemonicView
            hideIndex={createdIndex}
            mnemonic={mnemonic}
            handleClick={handleMnemonicClick}
          />
        </Grid>
        <Grid item xs={12}>
          <p>
        ...
        <Grid item xs={12}>
          <MnemonicView
            hideIndex={createdHideIndex}
            mnemonic={(createdMnemonic + ' ' + remainingWords.join(' ')).trim()}
            handleClick={handleCreatedMnemonicClick}
          />
        </Grid>
      </Grid>

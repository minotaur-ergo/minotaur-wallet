...
const ExtendedPublicKeyView = (props: ExtendedPublicKeyViewPropsType) => {
    const [encoding, setEncoding] = useState<"hex" | "base58" | "base64">("base58");
    const getDecoded = () => {
            ...
            <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                    ...
                    >
                        {["hex", "base58", "base64"].map((item, index) => (
                            <ToggleButton key={index} style={{ width: `33.33%` }} value={item}>
                                {item}
                            </ToggleButton>
                        ))}
                    </ToggleButtonGroup>
                ...
                <Grid item xs={12}>
                    <CopyToClipboard
                        ...
                        <div style={{ margin: 20, wordWrap: "break-word" }}>{getDecoded()}</div>
                    </CopyToClipboard>
                </Grid>
                <br />
                <br />
...
export default connect(mapStateToProps, mapDispatchToProps)(ExtendedPublicKeyView);

...
const Types = [
    {
        render: (param: string, close: () => any, completed?: (result: string) => any, wallet?: Wallet) =>
            <TransactionSignRequest completed={completed} tx={JsonBI.parse(param)} closeQrcode={close} />,
        type: TxSignR,
        detect: (value: string) => detectPagedWithPrefix(value, 'CSR'),
    }, {
        render: (param: string, close: () => any, completed?: (result: string) => any, wallet?: Wallet) =>
            <TransactionPublishRequest completed={completed} closeQrcode={close} tx={JsonBI.parse(param)} />,
        type: TxPublishR,

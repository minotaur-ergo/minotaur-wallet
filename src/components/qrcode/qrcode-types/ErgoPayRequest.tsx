...
export const ErgoPayQrCodeContext = createContext<QrCodeContextType | null>(
  null
);
...
class ErgoPayRequest extends React.Component<PropsType, stateType> {
  ...
  getUrlWithPrefix = (url: string) => {
    ...
    return (
      (hostname === 'localhost' || ip_regex.test(hostname)
        ? 'http://'
        : 'https://') + url
    );
  };
  ...
  getUrl = () => {
    ...
    if (
      ...
      this.urlContainPlaceHolder(this.props.url)
    ) {
        ...
        <ErgoPayQrCodeContext.Provider
          value={{
            ...
            showQrCode: (value: boolean) =>
              this.setState({ showQrCode: value }),
          ...
          >
            ...
            {this.state.status === 'Address' ? (
              <React.Fragment>
                <Container style={{ marginTop: 20 }}>
                  <Typography>
                    ErgoPay request needs one address. Please select one address
                    to use for it.
                  </Typography>
                </Container>
                <List>
              ...
              <React.Fragment>
                <Container style={{ marginTop: 20 }}>
                  <Typography>{this.state.response?.message}</Typography>
                </Container>
                {this.state.tx ? (
                  <TxView
                    network_type={

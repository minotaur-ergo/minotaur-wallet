...
class QrCodeReaderWeb extends React.Component<QrCodeWebPropsType, QrCodeWebStateType> {
    ...
    handleScan = (result: Result | undefined | null, error: Error | undefined | null) => {
        if (result) {
    ...
    componentDidUpdate = () => {
        if (this.videoRef.current) {
            if (!this.state.control) {
                codeReader.decodeFromVideoDevice(this.state.selectedDeviceId, this.videoRef.current, this.handleScan).then((control) => {
    ...
    render = () => {
        return (
            <WithAppBar header={<AppHeader hideQrCode={true} title="Scan Qrcode" back={this.props.closeQrcode}/>}>
                <Container style={{marginTop: 10, marginBottom: 10}}>
                    <FormControl fullWidth>
                        ...
                        <Select
                            ...
                            onChange={event => this.setState({selectedDeviceId: event.target.value})}
                        >
                            {this.state.videoInputDevices.map(videoDevice => (
                                <MenuItem key={videoDevice.deviceId}
                                          value={videoDevice.deviceId}>{videoDevice.label}</MenuItem>
                            ))}
                        </Select>
                    </FormControl>
                </Container>

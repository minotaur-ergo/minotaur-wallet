...
const ShareCommitmentMultiSig = (props: ShareCommitmentPropsType) => {
    ...
    return (
        <React.Fragment>
            {props.remainCount > 0 ? (
                <PublishManualType publishType={type} setPublishType={newType => setType(newType)} />
            ) : null}
            <Grid item xs={12}>
                {props.remainCount === 0 ? (
                    <React.Fragment>
                        <Typography>Transaction Signed Completely.</Typography>
                        <Button variant='contained' fullWidth color='primary' onClick={() => props.publishTx()}>
                            Send Transaction
                        </Button>
                    </React.Fragment>
                ) : (
                    <React.Fragment>
                        <Typography>
                            {props.required - props.remainCount} of {props.required} signatures added
                        </Typography>
                        <Typography>
                            Your sign added to transaction.
                            Share to other signers
                        </Typography>
                    </React.Fragment>
                )}
            </Grid>
            {props.remainCount > 0 && type === PUBLISH_MANUAL_TYPES.qrcode ? (
                <RequestQrcodeDisplay requestType={'MCR'} requestData={props.commitment} />
            ) : props.remainCount > 0 ?(

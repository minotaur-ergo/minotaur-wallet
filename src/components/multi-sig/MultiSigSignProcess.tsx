...
class MultiSigSignProcess extends React.Component<MultiSigSignProcessPropsType, MultiSigSignProcessStatesType> {
    ...
    acceptPassword = async (password: string) => {
        if (this.state.relatedWallet) {
            if (this.commitmentCount() + (this.state.myHints ? 0 : 1) >= parseInt(this.props.wallet.seed)) {
        ...
        return (
            <Container>
                <Grid container spacing={2} style={{ textAlign: 'center' }} marginTop={3}>
                    ...
                    {this.state.mySign ? (
                        <Grid xs={12} item>
                            <Button variant='contained' fullWidth color='primary' onClick={() => this.props.close()}>
                                Completed
                            </Button>
                        </Grid>

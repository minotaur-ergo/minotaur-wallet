...
class TokenName extends React.Component<PropsType, StateType> {
  ...
  loadToken = () => {
    if (
      !this.state.loading &&
      (this.state.loadedTokenId !== this.props.token_id ||
        this.state.loadedNetwork !== this.props.network_type)
    ) {
      const loadingTokenId = this.props.token_id;
      const loadingNetworkType = this.props.network_type;
      this.setState({ loading: true });
      AssetDbAction.getAssetByAssetId(loadingTokenId, loadingNetworkType).then(
        (tokens) => {
          this.setState({
            token: tokens,
            loading: false,
  ...
  render = () => {
    const name = this.state.token
      ...
      : this.props.token_id.substring(0, 5) +
        '...' +
        this.props.token_id.substring(this.props.token_id.length - 6);

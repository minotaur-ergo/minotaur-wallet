...
const erg_nano_erg_to_str = (erg: bigint, nano_erg: bigint, digits?: number, maxDigit?: number) => {
    ...
    while (nano_erg_str.length > digits && nano_erg_str.substring(nano_erg_str.length - 1) === "0")
    ...
    useEffect(() => {
        if (props.token) {
            if (!(token && token.asset_id === props.token)) {
                if (!loading) {
                    setLoading(true);
                    AssetDbAction.getAssetByAssetId(props.token, props.network_type).then(token => {
                        setToken(token);
                        setLoading(true);
                    });
                }

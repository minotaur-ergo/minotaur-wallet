...
const getAssetJsonFromWasm = (
  ...
  return Array(tokens.len())
    ...
    .filter((item) =>
      allowedAssets ? allowedAssets.indexOf(item.id().to_str()) >= 0 : true
    )
    .map((token) => {
      return {
        id: token.id().to_str(),
...
const getAssetJsonFromExplorer = (
  boxJs: InputBox,
  allowedAssets?: Array<string>
): Array<{ id: string; amount: bigint }> => {
  return boxJs.assets
    .filter((item) =>
      allowedAssets ? allowedAssets.indexOf(item.tokenId) >= 0 : true
    )
    .map((token) => {
...
const RawBox = (props: RawBoxPropsType) => {
  ...
  const assets = props.box
    ? getAssetJsonFromWasm(props.box)
    : props.boxJs
    ? getAssetJsonFromExplorer(props.boxJs)
    : [];
  const boxValue = props.box
    ? BigInt(props.box.value().as_i64().to_str())
    : props.boxJs
    ? props.boxJs.value
    : BigInt('0');
  const address = props.box
    ? wasm.Address.recreate_from_ergo_tree(props.box.ergo_tree()).to_base58(
        network_type.prefix
      )
    : props.boxJs
    ? props.boxJs.address

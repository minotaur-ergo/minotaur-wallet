...
class BlockChainActionClass {
  ...
  processResult = async (
    ...
    try {
      ...
      await BlockDbAction.InsertHeaders(
        Object.entries(blocks).map((value) => {
  ...
  calcForkPoint = async (
    ...
    dbHeaders
      ...
      .forEach((item) => (dbBlocks[item.height] = item.block_id));
    const dbHeight =
    ...
    while (minHeight > height - CONFIRMATION_HEIGHT) {
      const chunkHeaders = (
        await network_type.getExplorer().getBlocksHeaders(paging)
      ).items;
...
class BlockChainTxActionClass {
  getMinedTxForAddress = async (address: Address) => {
    ...
    const blocks = Object.assign(
      {},
      ...blocksList.map((item) => ({ [item.height]: item.id }))
    );
    ...
    while (!breakProcess) {
      ...
      for (const tx of txs) {
        if (tx.inclusionHeight > address.process_height) {
          ...
          if (
            tx.inclusionHeight >= minBlockHeight &&
  ...
  processAddressOutputBoxes = async (
    address: Address,
    height: number,
  ...
  processAddressInputBoxes = async (
    address: Address,
    height: number,
    txs: Array<TxWithJson>
  ) => {
...
export { Receiver, ReceiverToken, BlockChainAction, BlockChainTxAction };

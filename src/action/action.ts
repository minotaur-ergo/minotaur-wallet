...
import publicKeys from '../components/pages/wallet-add/multisig/PublicKeys';
...
class MultiSigActionClass {
    getMultiSigWalletProver = async (multiSigWallet: Wallet, signingWallet: Wallet, password: string) => {
    ...
    getMultiSigWalletPublicKeys = async (multiSigWallet: Wallet, signingWallet: Wallet) => {
        const addresses = await AddressDbAction.getWalletAddresses(multiSigWallet.id);
        ...
        addresses.forEach(address => {
            res[address.address] = derive_path.map(derive_key => derive_key.derive(address.idx).publicKey.toString('hex')).sort();
        });
        return res;
    };
    ...
    getMultiSigWalletMyPublicKeys = async (multiSigWallet: Wallet, signingWallet: Wallet) => {
        ...
        addresses.forEach(address => {
            res[address.address] = derive_path.derive(address.idx).publicKey.toString('hex');
        });
        return res;
    };

    getAddressToSecretMap = async (multiSigWallet: Wallet, signingWallet: Wallet, password: string) => {
        const addresses = await AddressDbAction.getWalletAddresses(multiSigWallet.id);
        const seed = decrypt(signingWallet.seed, password);
        ...
        for (let address of addresses) {
            ...
            const path = calcPathFromIndex(address.idx === -1 ? 0 : address.idx);
            const ergoTree = wasm.Address.from_base58(address.address).to_ergo_tree().to_base16_bytes();
            res[ergoTree] = await AddressAction.getSecretFromPath(seed, path);
        }
        return res;
    };

    generateCommitments = async (wallet: wasm.Wallet, tx: wasm.ReducedTransaction) => {
    ...
    commitmentToByte = (commitment: wasm.TransactionHintsBag, inputPublicKeys: Array<Array<string>>): Array<Array<string>> => {
        ...
        return inputPublicKeys.map((rowPublicKeys, index) => {
            ...
            hints.forEach((item: { pubkey: { h: string }, a: string }) => {
                ...
                if (pubIndex >= 0)
                    rowCommitments[pubIndex] = Buffer.from(item.a, 'hex').toString('base64');
            });
    ...
    generateHintBagJson = (publicKey: string, commitment: string, index: number) => {
        return {
            hint: 'cmtReal',
            pubkey: {
                op: "205",
    ...
    getHintBags = (publicKeys: Array<Array<string>>, commitments: Array<Array<string>>): wasm.TransactionHintsBag => {

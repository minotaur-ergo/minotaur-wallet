...
class AddressActionClass {
    addWalletAddresses = async (wallet: Wallet) => {
        ...
        let addressObject = await this.deriveAddress(wallet.extended_public_key, network_type.prefix, 0);
        await AddressDbAction.saveAddress(wallet, "Main Address", addressObject.address, addressObject.path, 0);
        try {
            let index = 1;
            while (true) {
                addressObject = await this.deriveAddress(wallet.extended_public_key, network_type.prefix, index);

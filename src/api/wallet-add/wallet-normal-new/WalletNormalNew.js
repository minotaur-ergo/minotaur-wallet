...
class WalletNormalNew extends React.Component {
    ...
    saveWallet = () => {
        createNormalWallet(this.state.name, this.state.mnemonic, "not available", "no address", "m/0");
    ...
    render = () => {
        return (
            <React.Fragment>
                ...
                {this.state.step === 0 ? (
                    <Name {...this.state} goBack={this.props.back} goForward={({name, mnemonic}) => this.gotoMnemonic(name, mnemonic)} confirm={true}>
                ...
                {this.state.step === 1 ? (
                    <NewWalletMnemonic {...this.state} goBack={() => this.setState({step:0, mnemonic: ''})} goForward={mnemonic => {this.setState({step:2, mnemonic: mnemonic})}}/>
                ) : null}
                {this.state.step === 2 ? (

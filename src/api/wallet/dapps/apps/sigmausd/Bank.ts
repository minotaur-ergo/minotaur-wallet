...
const reserve_ratio = (base_reserves: bigint, circulating_stable_coins: bigint, oracle_rate: bigint) => {
    if(base_reserves === BigInt(0) || oracle_rate === BigInt(0)) return 0
    if(circulating_stable_coins === BigInt(0)) return (base_reserves * BigInt(100)) / (oracle_rate)
    const per_stable_coin_rate = (base_reserves * BigInt(100)) / circulating_stable_coins;
...
class Bank {
    ...
    current_reserve_ratio = () => {
        return reserve_ratio(this.base_reserve(), this.num_circulating_stable_coins(), this.oracle.datapoint_in_cents());
    };
    ...
    liabilities = () => {
        ...
        return base_reserves_needed < base_reserve ? base_reserves_needed : base_reserve;
    };
    ...
    stable_coin_nominal_price = () => {
        ...
        return (num_stable_coins === BigInt(0) || oracle_rate < liabilities / num_stable_coins) ? oracle_rate : liabilities / num_stable_coins;
    };
    ...
    mint_stable_coin_reserve_ratio = (amount: bigint) => {
        const new_base_reserve = this.base_reserve() + this.base_cost_to_mint_stable_coin(amount);
        return reserve_ratio(new_base_reserve, this.num_circulating_stable_coins() + amount, this.oracle.datapoint_in_cents());
    };
    ...
    mint_reserve_coin_reserve_ratio = (amount: bigint) => {
        let new_base_reserve = this.base_reserve() + this.base_cost_to_mint_reserve_coin(amount);
        return reserve_ratio(new_base_reserve, this.num_circulating_stable_coins(), this.oracle.datapoint_in_cents());
    };
    ...
    num_able_to_redeem_reserve_coin = () => {
        ...
        while (low < high) {
            ...
            if (new_reserve_ratio === parameters.MIN_RESERVE_RATIO) return mid;
            if (new_reserve_ratio < parameters.MIN_RESERVE_RATIO) high = mid - BigInt(1);
    ...
    total_cost_to_mint_stable_coin = (amount: bigint, transaction_fee: bigint) => {
        const base_cost = this.base_cost_to_mint_stable_coin(amount);
        return base_cost + transaction_fee + parameters.MIN_BOX_VALUE * BigInt(2) + parameters.IMPLEMENTOR_FEE(base_cost);
    };
    ...
    fees_from_minting_stable_coin = (amount: bigint, transaction_fee: bigint) => {
        ...
        const protocol_fee = fee_less_amount + parameters.PROTOCOL_FEE(fee_less_amount);
        const implementor_fee = parameters.IMPLEMENTOR_FEE(fee_less_amount + protocol_fee);
    ...
    total_cost_to_mint_reserve_coin = (amount: bigint, transaction_fee: bigint) => {
        const base_cost = this.base_cost_to_mint_reserve_coin(amount);
        return base_cost + transaction_fee + parameters.MIN_BOX_VALUE * BigInt(2) + parameters.IMPLEMENTOR_FEE(base_cost);
    };

    fees_from_minting_reserve_coin = (amount: bigint, transaction_fee: bigint) => {
        const fee_less_amount = this.reserve_coin_nominal_price() * amount;
        const protocol_fee = parameters.PROTOCOL_FEE(fee_less_amount);
        const implementor_fee = parameters.IMPLEMENTOR_FEE(this.base_cost_to_mint_stable_coin(amount));
        return transaction_fee + protocol_fee + implementor_fee;
    };
    ...
    total_amount_from_redeeming_reserve_coin = (amount: bigint, transaction_fee: bigint) => {
    ...
    fees_from_redeeming_reserve_coin = (amount: bigint, transaction_fee: bigint) => {
        const fee_less_amount = this.reserve_coin_nominal_price() * amount;
        const protocol_fee = parameters.PROTOCOL_FEE(fee_less_amount);
        const implementor_fee = parameters.IMPLEMENTOR_FEE(this.base_amount_from_redeeming_reserve_coin(amount));
        return transaction_fee + protocol_fee + implementor_fee;
    };
    ...
    total_amount_from_redeeming_stable_coin = (amount: bigint, transaction_fee: bigint) => {
    ...
    fees_from_redeeming_stable_coin = (amount: bigint, transaction_fee: bigint) => {
        const fee_less_amount = this.stable_coin_nominal_price() * amount;
        const protocol_fee = parameters.PROTOCOL_FEE(fee_less_amount);
        const implementor_fee = parameters.IMPLEMENTOR_FEE(fee_less_amount + protocol_fee);
    ...
    create_candidate = (height: number, adding_stable_count: bigint, adding_reserve_count: bigint) => {
        ...
        if(adding_stable_count > BigInt(0)){
            adding_erg_count += this.base_cost_to_mint_stable_coin(adding_stable_count);
        }else if(adding_stable_count < BigInt(0)) {
            adding_erg_count -= this.base_amount_from_redeeming_stable_coin(-adding_stable_count);
        }else if(adding_reserve_count > BigInt(0)){
            adding_erg_count += this.base_cost_to_mint_reserve_coin(adding_reserve_count);
        }else if(adding_reserve_count < BigInt(0)){
            adding_erg_count -= this.base_amount_from_redeeming_reserve_coin(-adding_reserve_count);
        }else {
            throw Error("one of stable or reserve tokens must change")
        }
        const bankOutBuilder = new wasm.ErgoBoxCandidateBuilder(
            wasm.BoxValue.from_i64(wasm.I64.from_str((old_value + adding_erg_count).toString())),
            wasm.Contract.pay_to_address(wasm.Address.recreate_from_ergo_tree(this.box.ergo_tree())),
            height
        )
        ...
        bankOutBuilder.add_token(
            stable_token.id(),
            wasm.TokenAmount.from_i64(wasm.I64.from_str((BigInt(stable_token.amount().as_i64().to_str()) - adding_stable_count).toString()))
        )
        bankOutBuilder.add_token(
            reserve_token.id(),
            wasm.TokenAmount.from_i64(wasm.I64.from_str((BigInt(reserve_token.amount().as_i64().to_str()) - adding_reserve_count).toString()))
        )
        bankOutBuilder.add_token(nft_token.id(), nft_token.amount());
        bankOutBuilder.set_register_value(4,
            wasm.Constant.from_i64(
                wasm.I64.from_str((this.num_circulating_stable_coins() + adding_stable_count).toString())
            )
        )
        bankOutBuilder.set_register_value(5,
            wasm.Constant.from_i64(
                wasm.I64.from_str((this.num_circulating_reserve_coins() + adding_reserve_count).toString())
            )
        )
